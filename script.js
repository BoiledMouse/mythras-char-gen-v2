...
(function() {
  ...
  function updateCultureOptions() {
    ...
    // Update culture pool (points to allocate to culture skills)
    character.pools.culture = def.standard.length * 5;
    updatePoolsDisplay();
    updateSkillTable();
    updateSummary();
  ...
  function updateCareerOptions() {
    ...
    // Set career pool equal to number of standard skills * 5
    const def = careers[key];
    character.pools.career = (def.standard.length) * 5;
    updatePoolsDisplay();
    updateSkillTable();
    updateSummary();
  ...
  function rollAttributes() {
    ['STR','CON','SIZ','DEX','INT','POW','CHA'].forEach(key => {
      character.attributes[key] = rollDice(3, 6);
    });
  ...
  function updateSummary() {
    ...
    // Flatten skill totals
    const rows = [];
    const allSkills = Object.keys(standardSkills)
      .concat(...Object.values(cultures).map(c => c.standard),
              ...Object.values(cultures).map(c => c.professional),
              ...Object.values(careers).map(c => c.standard),
              ...Object.values(careers).map(c => c.professional));
    const seen = new Set();
    ...
    sum.innerHTML = `
      ...
      <p><strong>Culture:</strong> ${cultures[character.culture].name}</p>
      <p><strong>Career:</strong> ${careers[character.career].name}</p>
      <p><strong>Age:</strong> ${character.age}</p>
      <p><strong>Social Class:</strong> ${character.socialClass || '-'}</p>
      <p><strong>Combat Style:</strong> ${character.combatStyle || '-'}</p>
      <p><strong>Silver:</strong> ${character.money} sp</p>
