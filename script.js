...
(function() {
  ...
  // Social class multipliers for starting silver.  Higher social
  // standing multiplies the basic roll.  These values are chosen to
  // represent roughly one, two or three portions of starting money.
  ...
  function buildSkillsTable() {
    ...
    allSkills.forEach(skill => {
      ...
      baseCell.textContent = computeBaseForSkill(skill).toFixed(2);
      ...
      totalCell.textContent = computeTotalForSkill(skill).toFixed(2);
  ...
  function updateSkillBases() {
    ...
    skillsTableBody.querySelectorAll('tr').forEach(row => {
      ...
      const skill = cells[0].textContent;
  ...
  function generateStartingSilver() {
    ...
    const base = roll * 10;
    const total = base * socialMultiplier[character.socialClass];
  ...
  function exportToExcel() {
    ...
    Object.entries(data.skills).forEach(([skill,obj]) => {
      rows.push([skill,obj.base,obj.culture,obj.career,obj.bonus,obj.total]);
    });
    rows.push([]);
  ...
  function exportToMarkdown() {
    ...
    md += `|------|-----:|-------:|------:|-----:|------:|\n`;
    Object.entries(data.skills).forEach(([skill,obj]) => {
      md += `| ${skill} | ${obj.base.toFixed(1)} | ${obj.culture} | ${obj.career} | ${obj.bonus} | ${obj.total.toFixed(1)} |\n`;
    });
    md += `\n## Equipment\n`;
